...
const LoginForm = () => {
  // 1. We initialize the hook with our server action and an empty initial state
  const [state, action, isPending] = useActionState(loginAction, { message: '' });

  return (
    // 2. We pass the 'action' returned by the hook, NOT the import directly
    <form action={action} className="space-x-4">
